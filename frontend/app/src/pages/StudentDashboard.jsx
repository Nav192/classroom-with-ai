  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
      ...
      {quizzes.length > 0 ? (
        <ul className="space-y-3">
          {quizzes.map(quiz => (
            <li key={quiz.id} className="p-4 rounded-lg bg-gray-50 flex justify-between items-center">
              <div>
                <p className="font-semibold text-gray-800">{quiz.topic}</p>
                <p className="text-sm text-gray-500">Type: {quiz.type}</p>
              </div>
              <Link to={`/student/quiz/${quiz.id}`} className="bg-blue-600 text-white py-2 px-4 rounded-full hover:bg-blue-700 transition-colors flex items-center gap-2">
                <PlayCircle size={18} />
                Start Quiz
              </Link>
            </li>
      ...
      ) : <p className="text-gray-500">No quizzes available in this class yet.</p>}
    </div>
  );
}
